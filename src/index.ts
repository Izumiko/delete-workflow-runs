...
const delRunsForRepo = async (repo: string, token: string, retain_days: number, keep_minimum_runs: number) => {
...
const run = async () => {
  try {
    ...
    for (const row of parsed as string[][]) {
      if (row.length === 1) {
        repos.push(...row[0].split(','))
      } else {
        repos.push(...row)
      }
    }
    repos = repos.filter(r => r).map(r => r.trim())

    console.log(`Delete workflow runs older than ${retain_days} days for repository ${repos}`)

    for (const repo of repos)
      await delRunsForRepo(repo, token, retain_days, keep_minimum_runs)
